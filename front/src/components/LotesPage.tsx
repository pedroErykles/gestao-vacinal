...
} from "../services/fornecedores/fornecedores";

import { AsyncSearchSelect } from "../components/AsyncSearchSelect";

const formatarCNPJ = (cnpj: string | number | undefined | null): string => {
  if (!cnpj) return "";

  const cnpjLimpo = cnpj.toString().replace(/\D/g, "");

  const cnpjPad = cnpjLimpo.padStart(14, "0");

  return cnpjPad.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})$/, "$1.$2.$3/$4-$5");
};
...
export function LotesPage() {
  ...
  const [selectedVacinaObj, setSelectedVacinaObj] =
    useState<SearchVacinaResponse | null>(null);
  const [selectedFornecedorObj, setSelectedFornecedorObj] =
    useState<FornecedorResponse | null>(null);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      listaVacinas.forEach((v) => {
        if (v.fabricante) {
          fabsMap.set(v.fabricante.cnpj, {
            id: v.fabricante.cnpj,
            nome: v.fabricante.nome,
          });
        }
      });
  ...
  const getVacinaNome = (vacinaId: string) => {
    return (
      vacinas.find((v) => v.codigo_vacina.toString() === vacinaId)?.nome ||
      "N/A"
    );
  };
  ...
  const lotesVencidos = lotes.filter((l) =>
    isLoteVencido(l.dataValidade),
  ).length;
  const lotesProximoVencimento = lotes.filter((l) =>
    isLoteProximoVencimento(l.dataValidade),
  ).length;
    ...
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Controle e gerenciamento de lotes de vacinas
        </p>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl mt-2">
                {isLoading ? <Loader2 className="animate-spin" /> : totalLotes}
              </p>
            </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl mt-2">
                {isLoading ? "..." : totalDosesDisponiveis.toLocaleString()}
              </p>
            </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl mt-2">
                {isLoading ? "..." : lotesProximoVencimento}
              </p>
            </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-4">
            ...
            >
              ...
              {vacinas.map((vacina) => (
                <option key={vacina.codigo_vacina} value={vacina.codigo_vacina}>
                  {vacina.nome}
                </option>
              ))}
            </select>
                ...
                filteredLotes.map((lote) => (
                  <tr key={lote.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600">
                      {getFabricanteNome(lote.fabricanteId)}
                    </td>
                    ...
                    <td className="px-6 py-4">{getStatusBadge(lote)}</td>
                    <td className="px-6 py-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <AsyncSearchSelect<SearchVacinaResponse>
                    ...
                    onSelect={(v) => {
                      if (v && v.id) {
                        setFormData({ ...formData, vacinaId: v.id.toString() });
                      } else {
                ...
                <div>
                  ...
                  <AsyncSearchSelect<FornecedorResponse>
                    ...
                    getDisplayValue={(f) => `${f.nome} - ${formatarCNPJ(f.cnpj)}`}
                    renderItem={(f) => (
                      <div className="flex flex-col">
                        <span className="font-medium text-gray-900">
                          {f.nome}
                        </span>
                        <span className="text-xs text-gray-500">{formatarCNPJ(f.cnpj)}</span>
                      </div>
                    )}
                    onSelect={(f) => {
                      if (f) {
